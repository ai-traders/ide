describe "ide --rm false"
  ...
  describe 'when IDE_DRIVER="docker"'
    describe 'when --dryrun and --rm false is set'
  ...
  describe 'when IDE_DRIVER="docker-compose"'
    describe 'when --dryrun and --rm false is set'
      it "returns 1; not implemented"
        message="$(cd ${publicide_path_dc} && ${IDE_PATH} --rm false --dryrun)"
        assert equal "$?" "1"
        assert do_match "$message" "Not implemented feature: '--rm false' for driver: docker-compose"
      end
    end
  end
end
